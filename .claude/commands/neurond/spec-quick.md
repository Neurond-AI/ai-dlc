...
2. **Check Uniqueness**:
   - Use Glob to check `.neurond/specs/*/`
   - If feature name exists, append `-2`, `-3`, etc.

3. **Create Directory**:
   - Use Bash: `mkdir -p .neurond/specs/{feature-name}`

4. **Initialize Files from Templates**:

   a. Read templates:
   ```
   - .neurond/settings/templates/specs/init.json
   - .neurond/settings/templates/specs/requirements-init.md
   ```
   ...
   ```
   - .neurond/specs/{feature-name}/spec.json
   - .neurond/specs/{feature-name}/requirements.md
   ```
...
6. **Output Progress**:
   ```
   ✅ Spec initialized at .neurond/specs/{feature-name}/
   ```
...
```
/neurond:spec-requirements {feature-name}
```
...
**Execute SlashCommand**:
```
/neurond:spec-design {feature-name} -y
```
...
**Execute SlashCommand**:
```
/neurond:spec-tasks {feature-name} -y
```
...
### Phase 1 Tools
- **Glob**: Check `.neurond/specs/*/` for existing feature names
- **Bash**: Create directory with `mkdir -p`, generate timestamp with `date -u`
- **Read**: Fetch templates from `.neurond/settings/templates/specs/`
- **Write**: Create `spec.json` and `requirements.md` in spec directory

### Phase 2-4 Tools
- **SlashCommand**: Execute `/neurond:spec-requirements`, `/neurond:spec-design`, `/neurond:spec-tasks`
...
```
✅ Spec initialized at .neurond/specs/{feature}/
✅ Requirements generated → Continuing to design...
✅ Design generated → Continuing to tasks...
```
...
## Generated Files:
- .neurond/specs/{feature}/spec.json
- .neurond/specs/{feature}/requirements.md ({X} requirements)
- .neurond/specs/{feature}/design.md ({Y} components, {Z} endpoints)
- .neurond/specs/{feature}/tasks.md ({N} tasks)

⚠️ Quick generation skipped:
- `/neurond:validate-gap` - Gap analysis (integration check)
- `/neurond:validate-design` - Design review (architecture validation)

## Next Steps:
1. Review generated specs (especially design.md)
2. Optional validation:
   - `/neurond:validate-gap {feature}` - Check integration with existing codebase
   - `/neurond:validate-design {feature}` - Verify architecture quality
3. Start implementation: `/neurond:spec-impl {feature}`

## Note:
For complex features (integrations, security, APIs), use standard workflow:
/neurond:spec-init → /neurond:spec-requirements → /neurond:validate-gap
→ /neurond:spec-design → /neurond:validate-design → /neurond:spec-tasks
```
...
**Template Missing**:
- Check `.neurond/settings/templates/specs/` exists
...
- Suggest: "Continue manually from `/neurond:spec-{next-phase} {feature}`"
