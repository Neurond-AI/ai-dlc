...
## Execution Steps
1. **Check Uniqueness**: Verify `.neurond/specs/` for naming conflicts (append number suffix if needed)
2. **Create Directory**: `.neurond/specs/[feature-name]/`
3. **Initialize Files Using Templates**:
   - Read `.neurond/settings/templates/specs/init.json`
   - Read `.neurond/settings/templates/specs/requirements-init.md`
...
4. **Next Step**: Command block showing `/neurond:spec-requirements <feature-name>`
...
- **Template Missing**: If template files don't exist in `.neurond/settings/templates/specs/`, report error with specific missing file path and suggest checking repository setup
