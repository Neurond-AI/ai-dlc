...
- Glob(`.neurond/settings/templates/steering-custom/*.md`) to find available templates
...
2. **Check if template exists**:
   - Load from `.neurond/settings/templates/steering-custom/{name}.md` if available
...
4. **Generate custom steering**:
   ...
   - Apply principles from `.neurond/settings/rules/steering-principles.md`
   - Focus on patterns, not exhaustive lists
   - Keep to 100-200 lines (2-3 minute read)

5. **Create file** in `.neurond/steering/{name}.md`

## Available Templates

Templates available in `.neurond/settings/templates/steering-custom/`:
...
## Created:
- .neurond/steering/api-standards.md
...
- Light references to `.neurond/specs/` and `.neurond/steering/` are acceptable; avoid other `.neurond/` directories
