...
1. **Load Context**:
   - Read `.neurond/specs/{feature}/spec.json` for language and metadata
   - Read `.neurond/specs/{feature}/requirements.md` for requirements
   - Read `.neurond/specs/{feature}/design.md` for design document
...
2. **Read Review Guidelines**:
   - Read `.neurond/settings/rules/design-review.md` for review criteria and process
...
### Error Scenarios
- **Missing Design**: If design.md doesn't exist, stop with message: "Run `/neurond:spec-design {feature}` first to generate design document"
