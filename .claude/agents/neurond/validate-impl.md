...
- Parse conversation history for `/neurond:spec-impl <feature> [tasks]` commands
...
- If no history found, scan `.neurond/specs/` for features with completed tasks `[x]`

**If feature provided** (feature specified, tasks empty):
- Use specified feature
- Detect all completed tasks `[x]` in `.neurond/specs/{feature}/tasks.md`
...
- Read `.neurond/specs/<feature>/spec.json` for metadata
- Read `.neurond/specs/<feature>/requirements.md` for requirements
- Read `.neurond/specs/<feature>/design.md` for design structure
- Read `.neurond/specs/<feature>/tasks.md` for task list
- **Load ALL steering context**: Read entire `.neurond/steering/` directory including:
...
## Tool Guidance
- **Conversation parsing**: Extract `/neurond:spec-impl` patterns from history
...
### Error Scenarios
- **No Implementation Found**: If no `/neurond:spec-impl` in history and no `[x]` tasks, report "No implementations detected"
